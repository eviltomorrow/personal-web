...
const modules: Module[] = [
  ...
  { key: "balance-sheet",icon: I.Chart,    label: "资产负债", desc: "财务状况概览",     stat: "2 报表",  gradient:"from-[#34c759]/10 via-[#68d88b]/5 to-transparent", shadow:"shadow-[0_0_20px_rgba(52,199,89,0.08)]",  accent:"text-[#34c759] bg-[#34c759]/10", route: "/balance-sheet" },
];
...
function QuickAction({ label, desc, icon, color, onClick }: { label: string; desc: string; icon: ReactNode; color: string; onClick?: () => void }) {
  return (
    <button onClick={onClick} className="group flex items-center gap-3 rounded-xl px-4 py-3 transition-all duration-200 hover:bg-[#f5f5f7] cursor-pointer">
...
export default function HomePage() {
  ...
  function handleModuleClick(m: Module) {
    if (m.route) {
      router.push(m.route);
    }
  }
  ...
  const totalBookmarks = 128;
  const totalPhotos = 346;
          ...
          <div className="mx-auto max-w-[1000px] px-8 pt-8 pb-16">
            ...
            <div className="mb-8">
              <div className="mb-3 flex items-center gap-2">
                <h2 className="text-[15px] font-semibold tracking-[-0.01em] text-[#1d1d1f]">数据概览</h2>
                <div className="h-px flex-1 bg-gradient-to-r from-[#e8e8ed] to-transparent" />
              </div>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <MiniStat label="笔记"   value={`${totalNotes}`}   trend="+3"   color="text-[#0071e3]" />
                <MiniStat label="项目"   value={`${totalProjects}`} trend="+1"   color="text-[#30b94e]" />
                <MiniStat label="收藏"   value={`${totalBookmarks}`} trend="+5"   color="text-[#ff9f0a]" />
                <MiniStat label="照片"   value={`${totalPhotos}`}   trend="+12"  color="text-[#ff3b30]" />
              </div>
            </div>
            ...
            <div className="mb-8">
              <div className="mb-3 flex items-center gap-2">
                <h2 className="text-[15px] font-semibold tracking-[-0.01em] text-[#1d1d1f]">所有功能</h2>
                <span className="rounded-full bg-[#0071e3]/10 px-2.5 py-0.5 text-[10px] font-medium text-[#0071e3]">{modules.length} 项</span>
                <div className="h-px flex-1 bg-gradient-to-r from-[#e8e8ed] to-transparent" />
              </div>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {modules.map((m) => (
                  <button key={m.key} onClick={() => handleModuleClick(m)}
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
              ...
              <div className="rounded-2xl border border-[#e8e8ed]/80 bg-white p-5 shadow-[0_1px_4px_rgba(0,0,0,0.04)] lg:col-span-3 transition-all duration-200 hover:shadow-md">
                ...
                <div className="space-y-3">
                  ...
                  <Activity label="资产负债表已更新" module="资产负债" time="4 天前" icon={I.Chart} />
              ...
              <div className="rounded-2xl border border-[#e8e8ed]/80 bg-white p-5 shadow-[0_1px_4px_rgba(0,0,0,0.04)] lg:col-span-2 transition-all duration-200 hover:shadow-md">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <QuickAction label="查看资产负债" desc="财务状况概览" icon={I.Chart} color="bg-[#34c759]/10 text-[#34c759]" onClick={() => router.push("/balance-sheet")} />
                </div>
              </div>
            </div>
